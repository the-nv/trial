...
adorfo = yf.Ticker('DLF.BO')
hist = adorfo.history(start="2008-03-21")

gannSquare = gann.GannSquare(100, np.datetime64(hist.index[0].date()))
...
fig3.update_yaxes(range=[0,30000000],secondary_y=True)
...
for i in range(len(angle_0)):
    if angle_0[i] < maximum:
        fig3.add_hline(y=angle_0[i], line_color='red', line_width=0.75, annotation_text="0")
        
    if date_angle_0[i] < maxDate:
        xx = (date_angle_0[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='red', line_width=0.75, annotation_text="0")
    

for i in range(len(angle_45)):
    if angle_45[i] < maximum:
...
for i in range(len(angle_90)):
    if angle_90[i] < maximum:
        fig3.add_hline(y=angle_90[i], line_color='green', line_width=0.75, annotation_text="90")
        
    if date_angle_90[i] < maxDate:
        xx = (date_angle_90[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='green', line_width=0.75, annotation_text="90")
    

for i in range(len(angle_135)):
    if angle_135[i] < maximum:
        fig3.add_hline(y=angle_135[i], line_color='black', line_width=0.75, annotation_text="135")
        
    if date_angle_135[i] < maxDate:
        xx = (date_angle_135[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='black', line_width=0.75, annotation_text="135")
    


for i in range(len(angle_180)):
    if angle_180[i] < maximum:
        fig3.add_hline(y=angle_180[i], line_color='pink', line_width=0.75, annotation_text="180")
    
    if date_angle_180[i] < maxDate:
        xx = (date_angle_180[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='pink', line_width=0.75, annotation_text="180")
    

for i in range(len(angle_225)):
    if angle_225[i] < maximum:
        fig3.add_hline(y=angle_225[i], line_color='violet', line_width=0.75, annotation_text="225")
        
    if date_angle_225[i] < maxDate:
        xx = (date_angle_225[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='violet', line_width=0.75, annotation_text="225")
    

for i in range(len(angle_270)):
    if angle_270[i] < maximum:
        fig3.add_hline(y=angle_270[i], line_color='brown', line_width=0.75, annotation_text="270")
    
    if date_angle_270[i] < maxDate:
        xx = (date_angle_270[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='brown', line_width=0.75, annotation_text="270")
    

for i in range(len(angle_315)):
    if angle_315[i] < maximum:
        fig3.add_hline(y=angle_315[i], line_color='purple', line_width=0.75, annotation_text="315")
    
    if date_angle_315[i] < maxDate:
        xx = (date_angle_315[i] - np.datetime64('1970-01-01'))/ np.timedelta64(1, 's')
        fig3.add_vline(x=xx * 1000, line_color='purple', line_width=0.75, annotation_text="315")
